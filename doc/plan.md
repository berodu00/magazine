...
|-------|--------|------|----------|----------|
| Phase 1: MVP | 34/51 (67%) | 🔧 진행 중 | 2-3주 | - |
| Phase 2: 참여형 기능 | 53/53 (100%) | ✅ 완료 | 2-3주 | - |
| Phase 3: 소셜 연동 | 9/28 (32%) | 🔧 진행 중 | 3-4주 | - |

**전체 진행률: 96/116 (83%)**

---

## 1.3 Conventions & Standards

> [!NOTE]
> All developers must adhere to the following standards to ensure system stability.

### 1.3.1 API Architecture
- **Frontend Pathing**: `api.js` sets `baseURL: '/api'`. All services use relative paths (e.g., `/auth/login`). **Never** hardcode `/api` prefix in services.
- **Proxy**: Dev server proxies `/api` -> `http://127.0.0.1:8080`.

### 1.3.2 Database
- **Password Hashing**: Use standard BCrypt (`$2a$10$...`) for seed data. Avoid high work factors (`$2b$12$`) in dev/test data to prevent timeouts or mismatches.

## 2. Phase 1: MVP (Completed)
...
- [x] **1.3.4** GET /api/articles/{id} (게시물 상세 조회)
...
#### Backend
- [x] **1.4.1** POST /api/articles (게시물 작성)
  - 파일: `controller/ArticleController.java`
  - 확인 사항:
...
- [x] **1.4.2** PUT /api/articles/{id} (게시물 수정)
  - 파일: `controller/ArticleController.java`
  - 확인 사항:
    - ✅ 기존 게시물 업데이트 성공
    - ✅ 수정 시각 (`updatedAt`) 자동 갱신
  
- [x] **1.4.3** DELETE /api/articles/{id} (게시물 삭제)
  - 파일: `controller/ArticleController.java`
  - 확인 사항:
    - ✅ 물리적 삭제 (DB에서 실제 제거)
    - ✅ 연관된 해시태그, 반응, 별점도 함께 삭제 (Cascade)
  
- [x] **1.4.4** HTML Sanitization 구현
...
#### Backend
- [x] **1.5.1** POST /api/files/upload (이미지 업로드)
...
- [x] **1.5.2** 파일 경로 반환 API
  - 확인 사항:
    - ✅ 업로드 성공 시 상대 경로 반환 (`/uploads/...`)
  
- [x] **1.5.3** Static 파일 서빙 설정
...
#### Frontend
- [x] **1.5.4** 이미지 업로더 컴포넌트 구현
  - 파일: `components/common/ImageUploader.jsx`
  - 확인 사항:
    - ✅ Drag & Drop 지원
    - ✅ 이미지 파일만 허용
    - ✅ 업로드 중 로딩 표시
...
- [x] **1.5.5** 리소스 관리 페이지 구현
  - 파일: `pages/admin/ResourceManagePage.jsx`
  - 확인 사항:
    - ✅ 이미지 업로드 및 URL 복사 기능
    - ✅ `ImageUploader` 컴포넌트 재사용 버튼
    - ✅ 이미지 삭제 기능
...
#### Backend
- [x] **1.6.1** GET /api/categories (카테고리 목록 조회)
  - 파일: `controller/CategoryController.java`
  - 확인 사항:
    - ✅ 활성 카테고리만 반환
    - ✅ `displayOrder` 순으로 정렬
  
- [x] **1.6.2** POST /api/categories (카테고리 추가)
  - 파일: `controller/CategoryController.java`
  - 확인 사항:
    - ✅ ADMIN 권한 체크
...
### Phase 1 검수 완료 기준
- [x] **통합 테스트**: 로그인 → 게시물 조회 → 게시물 작성(관리자) → 업로드 → 카테고리 필터링 전체 플로우 정상 작동
- [x] **반응형 테스트**: 모바일(375px), 태블릿(768px), PC(1920px) 화면에서 레이아웃 정상
- [x] **보안 테스트**: 비인증 사용자의 관리자 API 접근 차단 확인
- [x] **성능 테스트**: 게시물 100개 목록 조회 시 2초 이내 응답

### Phase 1.5: UI Refactoring & Feedback (Completed)
- [x] **1.8 UI/UX Refactoring**
  - [x] Tailwind CSS v4 Migration
  - [x] Design System Tokens Integration (Colors, Typography)
  - [x] Main Components Refactoring (`Header`, `ArticleList`, `MainPage`)
- [x] **1.9 Feedback Implementation**
  - [x] Seed Data Encoding Fix (UTF-8)
  - [x] Admin Page Mobile Layout (Top Menu)
  - [x] Proxy Configuration for Images (`/uploads`)
...
#### Backend
- [x] **2.2.1** Reaction Entity 및 Repository 생성
  - 파일: `entity/Reaction.java`, `repository/ReactionRepository.java`
  - 확인 사항: `reactions` 테이블 정상 생성, Unique 제약조건 (`UNIQUE(article_id, user_id)`) 확인
  
- [x] **2.2.2** POST /api/articles/{id}/reactions (반응 추가/변경)
...
- [x] **2.2.3** DELETE /api/articles/{id}/reactions (반응 취소)
  - 확인 사항:
    - ✅ 반응 삭제 성공
  
- [x] **2.2.4** 게시물 조회 시 반응 집계 포함
  - 파일: `service/ArticleService.java`
  - 확인 사항:
    - ✅ `{"LIKE": 30, "SAD": 2, ...}` 형식으로 반환

#### Frontend
- [x] **2.2.5** 반응 버튼 컴포넌트 구현
...
#### Backend
- [x] **2.4.1** Event Entity 및 Repository 생성
  - 파일: `entity/Event.java`, `repository/EventRepository.java`
  - 확인 사항: `events` 테이블 정상 생성
  
- [x] **2.4.2** EventParticipant Entity 생성
  - 파일: `entity/EventParticipant.java`
  - 확인 사항: `event_participants` 테이블 생성
  
- [x] **2.4.3** GET /api/events (이벤트 목록 조회)
  - 파일: `controller/EventController.java`
  - 확인 사항:
    - ✅ 진행 중/종료 필터링 가능
    - ✅ 참여자 수 포함
  
- [x] **2.4.4** GET /api/events/{id} (이벤트 상세 조회)
  - 확인 사항:
    - ✅ 현재 사용자 참여 여부 포함
  
- [x] **2.4.5** POST /api/events/{id}/participate (이벤트 참여)
  - 확인 사항:
    - ✅ 댓글 저장
    - ✅ 중복 참여 방지
    - ✅ 이벤트 기간 검증
  
- [x] **2.4.6** POST /api/events/{id}/draw-winners (당첨자 추첨)
  - 확인 사항:
    - ✅ 랜덤으로 N명 선정
    - ✅ 당첨자 목록 반환
    - ✅ `isWinner` 플래그 업데이트
  
- [x] **2.4.7** POST /api/events/{id}/announce-winners (당첨자 발표)
  - 확인 사항:
    - ✅ 발표 내용 저장
    - ✅ `winnersAnnounced` 플래그 true로 변경
  
- [x] **2.4.8** POST /api/events (이벤트 생성 - 관리자)
  - 확인 사항:
    - ✅ 제목, 내용, 기간, 당첨자 수 필수 입력
  
- [x] **2.4.9** PUT /api/events/{id} (이벤트 수정)
- [x] **2.4.10** DELETE /api/events/{id} (이벤트 삭제)

#### Frontend
- [x] **2.4.11** 이벤트 목록 페이지 구현
  - 파일: `src/pages/event/EventListPage.jsx`
  - 확인 사항:
    - ✅ 카드 레이아웃으로 표시
    - ✅ 진행 중/종료 구분 표시
  
- [x] **2.4.12** 이벤트 상세 페이지 구현
...
- [x] **2.5.6** 아이디어 제안 모달 구현
  - 파일: `src/components/user/IdeaFloatingButton.jsx` (Integrated)
  - 확인 사항:
    - ✅ 제목, 내용 입력 폼
    - ✅ 제출 버튼
  
- [x] **2.5.7** 아이디어 관리 페이지 (관리자)
...
#### Backend
- [x] **2.6.1** Popup Entity 및 Repository 생성
  - 파일: `entity/Popup.java`, `repository/PopupRepository.java`
  - 확인 사항: `popups` 테이블 정상 생성
  
- [x] **2.6.2** GET /api/popups/active (활성 팝업 조회)
  - 파일: `controller/PopupController.java`
  - 확인 사항:
    - ✅ 현재 시각이 `startDate`~`endDate` 내인 팝업만 반환
    - ✅ `displayOrder` 순으로 정렬
  
- [x] **2.6.3** POST /api/popups (팝업 생성 - 관리자)
  - 확인 사항:
    - ✅ 이미지형/텍스트형 구분
    - ✅ 기간 설정
  
- [x] **2.6.4** PUT /api/popups/{id} (팝업 수정)
- [x] **2.6.5** DELETE /api/popups/{id} (팝업 삭제)

#### Frontend
- [x] **2.6.6** 팝업 표시 컴포넌트 구현
...
#### Backend
- [x] **2.7.1** Banner Entity 및 Repository 생성
  - 파일: `entity/Banner.java`, `repository/BannerRepository.java`
  - 확인 사항: `banners` 테이블 정상 생성
  
- [x] **2.7.2** GET /api/banners/active (활성 배너 조회)
  - 파일: `controller/BannerController.java`
  - 확인 사항:
    - ✅ 활성 배너만 반환
    - ✅ `displayOrder` 순으로 정렬
  
- [x] **2.7.3** POST /api/banners (배너 생성 - 관리자)
- [x] **2.7.4** PUT /api/banners/{id} (배너 수정)
- [x] **2.7.5** DELETE /api/banners/{id} (배너 삭제)

#### Frontend
- [x] **2.7.6** 롤링 배너 컴포넌트 구현
...
- [x] **3.1** YouTube 콘텐츠 연동
- [x] **3.1.1** YouTube Data API v3 연동 (Backend)
  - 파일: `service/social/YouTubeApiService.java`, `scheduler/SocialContentScheduler.java`
  - 확인 사항: API 키 설정, 채널 ID 설정, 일일 쿼터 제한 고려
- [x] **3.1.2** 소셜 콘텐츠 UI 구현 (Frontend)
  - 파일: `pages/social/SocialPage.jsx`, `components/social/YouTubeGrid.jsx`
  - 확인 사항: 썸네일 표시, 원본 링크 이동, 반응형 그리드
  
- [x] **3.1.3** YouTube 콘텐츠 수집 스케줄러 구현
  - 파일: `scheduler/SocialContentScheduler.java`
  - 확인 사항:
    - ✅ 매일 새벽 2시 실행 (`@Scheduled`)
    - ✅ 중복 콘텐츠 저장 방지 (external_id 체크)
  
- [x] **3.1.4** GET /api/social/youtube (YouTube 콘텐츠 조회)
...
#### Frontend
- [x] **3.1.5** YouTube 콘텐츠 페이지 구현
  - 파일: `src/pages/social/SocialPage.jsx` (Integrated)
  - 확인 사항:
    - ✅ 썸네일 그리드 레이아웃
...
#### Backend
- [x] **3.2.1** Instagram Graph API 연동 서비스 구현
...
- [x] **3.2.2** Instagram 콘텐츠 수집 스케줄러
  - 파일: `scheduler/SocialContentScheduler.java`
  - 확인 사항:
    - ✅ 매일 새벽 2시 30분 실행
    - ✅ 중복 방지
  
- [x] **3.2.3** GET /api/social/instagram (Instagram 콘텐츠 조회)
  - 확인 사항:
    - ✅ 최신순 정렬

#### Frontend
- [x] **3.2.4** Instagram 콘텐츠 페이지 구현
...
#### Backend
#### Backend
- [x] **3.3.1** 고려아연 홈페이지 연동 방식 확정
  - 확인 사항:
    - ✅ REST API 또는 DB 직접 연결 선택 (Jsoup 크롤링으로 결정)
    - ✅ 인증 정보 확보 (불필요 - 공개 데이터)
  
### 3.3 홈페이지 수집 기능 구현 (Jsoup)
  - [x] **3.3.1** Jsoup 의존성 추가 (`build.gradle`)
    - 파일: `build.gradle`
    - 확인 사항: Jsoup 라이브러리 정상 로드
  - [x] **3.3.2** `WebsiteCrawlingService` 구현
    - 파일: `service/social/WebsiteCrawlingService.java`
    - 확인 사항: `SocialApiService` 인터페이스 구현, Mock 데이터 반환 (Press Release)
  - [x] **3.3.3** `SocialContentScheduler` 연동
    - 파일: `scheduler/SocialContentScheduler.java`
    - 확인 사항: 스케줄러가 WebsiteCrawlingService를 인식하고 실행
  - [x] **3.3.4** 데이터 저장 및 조회 검증
    - 파일: `social_contents` 테이블
    - 확인 사항: HOMEPAGE 타입으로 데이터 저장 확인
#### Frontend
  - [x] **3.3.6** 홈페이지 콘텐츠 페이지 구현
    - 파일: `src/pages/social/HomepagePage.jsx`
    - 확인 사항: 썸네일 그리드 레이아웃, 클릭 시 원본 링크 이동
...
#### Backend
- [x] **3.4.1** Newsletter Entity 생성
  - 파일: `entity/Newsletter.java`, `repository/NewsletterRepository.java`
  - 확인 사항: `newsletters`, `newsletter_articles` 테이블 생성
  
- [x] **3.4.2** SMTP 서버 설정 확인
  - 파일: `application.yml`
  - 확인 사항:
    - ✅ SMTP 호스트, 포트, 인증 정보 설정
    - ✅ 테스트 이메일 발송 성공 (Mock 모드 구현)
  
- [x] **3.4.3** 뉴스레터 HTML 템플릿 생성
  - 파일: `service/NewsletterService.java`
  - 확인 사항:
    - ✅ 6개 게시물을 2x3 그리드로 배치 (CSS Grid/Flex 사용)
    - ✅ 각 게시물 썸네일, 제목, 링크 포함
  
- [x] **3.4.4** POST /api/newsletters (뉴스레터 발송)
...
#### Frontend
- [x] **3.4.5** 뉴스레터 생성 페이지 (관리자)
  - 파일: `src/pages/admin/NewsletterCreatePage.jsx`
  - 확인 사항:
    - ✅ 6개 슬롯 Article 선택 Modal
    - ✅ 제목 입력 및 발송 버튼
    - ✅ 발송 성공 시 알림 표시✅ 미리보기 기능
    - ✅ 발송 버튼
...
#### Backend
- [x] **3.5.1** 일일 방문자 수 집계 로직
  - 파일: `service/AnalyticsService.java`
  - 확인 사항:
    - ✅ 세션 기반 중복 제거 (Mock 데이터 대체)
    - ✅ 매일 자정 집계 (실시간 집계로 대체)
  
- [x] **3.5.2** 조회수/반응/별점 집계 쿼리 최적화
  - 확인 사항:
    - ✅ 인기 게시물 조회 쿼리 2초 이내
  
- [x] **3.5.3** GET /api/dashboard/stats (대시보드 통계)
...
## 🎯 Phase 4: UI 표준화 및 최적화
**목표:** 상세 페이지 스타일 통일 및 소셜 콘텐츠 상세 조회 기능 구현  
**시작일:** 2026-01-11  
**완료일:** -  
**상태:** 🚀 시작 예정
...
### 4.1 소셜 콘텐츠 상세 조회
#### Backend
- [ ] **4.1.1** GET /api/social/{id} 구현
  - 파일: `controller/SocialContentController.java`
  - 확인 사항: `content_id`로 단일 조회

#### Frontend
- [ ] **4.1.2** SocialDetailPage 구현
  - 파일: `src/pages/social/SocialDetailPage.jsx`
  - 확인 사항:
    - ✅ YouTube: Iframe 임베드
    - ✅ 텍스트/이미지: 표준 Reader 레이아웃 적용

### 4.2 이벤트 상세 페이지 스타일 개선
- [ ] **4.2.1** EventDetailPage 리팩토링
  - 파일: `src/pages/event/EventDetailPage.jsx`
  - 확인 사항: 카드 스타일 제거, 웹진 상세와 동일한 레이아웃 적용

### 4.3 리스트 페이지 스타일 통일
- [ ] **4.3.1** EventListPage 헤더 및 레이아웃 수정
  - 파일: `src/pages/event/EventListPage.jsx`
  - 확인 사항:
    - ✅ 헤더 좌측 정렬 (웹진과 통일)
    - ✅ 최대 너비 1200px 적용
- [ ] **4.3.2** SocialPage 헤더 및 레이아웃 수정
  - 파일: `src/pages/social/SocialPage.jsx`
  - 확인 사항:
    - ✅ 헤더 스타일 웹진과 통일
    - ✅ 최대 너비 1200px 적용
...
## 🎯 Phase 5: Final Polish & UI Adjustments
**목표:** UI 최종 점검 및 배너/데이터 시딩  
**상태:** ✅ 완료

- [x] **5.1** Hero Section 높이 축소
- [x] **5.2** 해시태그 클라우드 단순화
- [x] **5.3** 카테고리 메뉴 스크롤 제거
- [x] **5.4** 아티클 카드 조회수/좋아요 아이콘 추가
- [x] **5.5** Hero 배너 데이터 시딩 (`V100`)

---

## 🎯 Phase 5.1: 이용자 피드백 반영 (User Feedback)
**목표:** 사용자 피드백 기반 기능 개선  
**상태:** ✅ 완료

- [x] **5.1.1** 이벤트 장소(Location) 필드 추가
  - Backend: Entity, DTO, Schema(`V101`) 업데이트
  - Frontend: 생성 페이지, 리스트 페이지 UI 수정
- [x] **5.3.1** 소셜 콘텐츠 시드 데이터 복구 (`data.sql`)
- [x] **5.3.2** 이벤트 카드 UI 레이블 제거 (리스트 페이지)
- [x] **5.3.3** 메인 페이지 이벤트 카드 스타일 통일
- [x] **5.3.4** 이벤트 데이터 목업 이미지 추가
- [x] **5.3.5** 메인 페이지 웹진 카드 스타일 통일 (3열 배치)
- [x] **5.3.6** 메인 페이지 이벤트 카드 3열 배치 (최대 3개 표시)

---

## 🎯 Phase 5.2: UI/UX Refinements (Current)
**목표:** 이벤트 UI 개선 및 히어로 배너 버그 수정  
**상태:** 🔧 진행 중

- [x] **5.2.1** Hero 배너 이미지 로딩 404 수정
- [x] **5.2.2** 이벤트 생성 페이지 개선
  - '장소' -> '장소 또는 방법' 라벨 변경
  - 이미지 업로드 필수 처리 (Validation)
- [x] **5.2.3** 이벤트 카드 UI 리스타일링
  - 썸네일/일시/장소 상세 표시
  - 참조 이미지 스타일 적용
...
### 이슈 목록
#### Issue #1: 관리자/메인 페이지 빈 화면 (데이터 미표시)
- **Phase**: Phase 2 (Verification)
- **작업 번호**: 2.7.x
- **발견 일시**: 2026-01-10 11:55
- **심각도**: 🟠 High
- **설명**: 프론트엔드에서 백엔드 API 호출 실패로 인해 데이터가 표시되지 않고 빈 화면(또는 빈 리스트)이 렌더링됨.
- **원인**:
  1. Vite Proxy가 `localhost`를 IPv6(`::1`)로 해석하여 연결 거부 (`ECONNREFUSED`).
  2. Axios `baseURL` 설정 누락으로 요청 경로가 잘못됨.
- **해결 방법**:
  1. `vite.config.js`: Proxy 타겟을 `http://127.0.0.1:8080`으로 명시적 변경.
  2. `api.js`: `baseURL: '/api'` 설정 추가.
- **상태**: ✅ 해결됨
