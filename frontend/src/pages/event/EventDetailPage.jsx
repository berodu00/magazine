        ...
        <div className="min-h-screen flex flex-col pt-16">
            ...
            <main className="flex-1 max-w-[800px] w-full mx-auto px-6 py-12">
                ...
                <div className="mb-12">
                    ...
                    <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-8 leading-tight">{event.title}</h1>

                    {event.thumbnailUrl && (
                        <div className="rounded-xl overflow-hidden mb-8 border border-gray-100">
                            <img src={event.thumbnailUrl} alt={event.title} className="w-full object-cover" />
                        </div>
                    )}
                ...
                {/* Participation Form */}
                {event.isActive && (
                    <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-8 border border-gray-200 shadow-sm mt-12">
                        ...
                        {event.isParticipated ? (
                            ...
                            <form onSubmit={handleParticipate} className="max-w-md mx-auto">
                                <textarea
                                    ...
                                    className="w-full h-32 p-4 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all resize-none mb-4 text-sm"
                                    disabled={submitting}
                                />
                                <button
                                    type="submit"
                                    disabled={submitting}
                                    className="w-full py-3 bg-primary text-white rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {submitting ? '처리중...' : '참여하기'}
                                </button>
                            </form>
                ...
                <div className="mt-12 text-center">
                    <button onClick={() => navigate('/events')} className="text-gray-500 hover:text-gray-900 font-medium">
                        목록으로 돌아가기
                    </button>
                </div>
            </main>
